...
fn cx_wine() -> PathBuf {
    PathBuf::from("/Applications/external runtime.app/Contents/SharedSupport/external runtime/lib/wine/x86_64-unix/wine")
}

fn cx_root() -> PathBuf {
    PathBuf::from("/Applications/external runtime.app/Contents/SharedSupport/external runtime")
}
...
pub fn status() -> Value {
    ...
    json!({
        ...
        "running": running,
        "external runtime_available": cx_available
    })
}
...
pub fn launch_game_via_steam(appid: u32) -> Result<Value, Box<dyn std::error::Error>> {
    let wine = cx_wine();
    if !wine.exists() {
        return Err("external runtime Wine not found".into());
    }
    if !is_wine_steam_running() {
        launch_wine_steam()?;
        for _ in 0..30 {
