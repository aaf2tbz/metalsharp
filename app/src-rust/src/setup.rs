...
pub fn dependencies() -> Value {
    ...
    let external runtime = check_path(&PathBuf::from(
    ...
    json!({
        ...
        "dependencies": [
            ...
            },
            {
                ...
                "desc": "external runtime Wine with built-in D3D/Vulkan rendering. Required for Among Us and other Unity 32-bit games that don't work with DXVK+MoltenVK.",
                "installed": external runtime,
                "required": false,
                "installCmd": "brew install --cask external runtime",
            },
        ],
    })
}
...
pub fn prepare_game(appid: u32) -> Result<Value, Box<dyn std::error::Error>> {
    ...
    match appid {
        ...
        945360 | 1139900 => prepare_external runtime_game(&game_dir, &home, appid)?,
...
fn prepare_external runtime_game(game_dir: &PathBuf, home: &PathBuf, appid: u32) -> Result<(), Box<dyn std::error::Error>> {
