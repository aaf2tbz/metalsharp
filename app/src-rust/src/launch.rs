...
fn find_scripts_dir() -> Option<PathBuf> {
    ...
    let candidates = vec![home.join("metalsharp").join("scripts"), crate::platform::metalsharp_home().join("scripts")];
...
pub fn run_game_setup_script(appid: u32) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", &env_path)
        .env("HOME", &home)
        .env("METALSHARP_HOME", crate::platform::metalsharp_home().to_string_lossy().to_string())
