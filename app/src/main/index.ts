...
function uiOnlyBackendResponse(method: string, url: string): unknown {
  ...
  if (url === "/runtime/diagnostics") {
    ...
    return {
      ...
      vulkan: {
        ok: false,
        readOnly: true,
        runtimeLibraryRoot: path.join(metalsharpHome, "runtime", "wine", "lib"),
        dxvk: {
          ok: false,
          root: path.join(metalsharpHome, "runtime", "wine", "lib", "dxvk"),
          present: 0,
          total: 9,
          missing: 9,
          entries: [],
        ...
        vkd3d: {
          ok: false,
          root: path.join(metalsharpHome, "runtime", "wine", "lib", "vkd3d"),
          present: 0,
          total: 4,
          missing: 4,
          entries: [],
      ...
      lanes: {
        ...
        entries: [
          ...
          },
          {
            ...
            artifactSummary: { total: 7, present: 7, missing: 0, allPresent: true },
          },
          {
            id: "d3dmetal_native",
            ...
            status: "reserved",
            ready: false,
            blockers: ["lane_reserved", "host_abi_missing", "payload_missing"],
            requiresWine: true,
            sourceScopes: ["steam", "gog"],
            runtimeSurfacePaths: ["runtime/wine/lib/d3dmetal_native"],
            artifactSummary: null,
          },
        ],
      },
  ...
  if (url === "/d3dmetal-native/setup-status" || url === "/d3dmetal-native/stage" || url === "/d3dmetal-native/patch") {
    return {
      ok: true,
      download_url:
      ...
      actions: [
        { id: "install_framework", label: "Install Framework", enabled: true, state: "missing", detail: "Open Apple Developer download for GPTK4 evaluation environment" },
        { id: "stage", label: "Stage", enabled: false, state: "missing", detail: "Download the GPTK4 evaluation DMG before staging" },
