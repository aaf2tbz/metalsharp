...
class App {
  ...
  private async loadLibrary() {
    ...
    const steamStatus = await this.api<{ installed: boolean; running: boolean; crossover_available: boolean }>("GET", "/steam/status");
    this.wineSteamInstalled = steamStatus?.installed ?? false;
    this.wineSteamRunning = steamStatus?.running ?? false;
    this.crossoverAvailable = steamStatus?.crossover_available ?? false;
  ...
  private renderSettings() {
    ...
    el.innerHTML = `
      ...
      <div class="settings-section">
        ...
        <div class="settings-row">
          <div>
            <div class="settings-label">Wine Steam (CrossOver)</div>
            <div class="settings-desc">Windows Steam running in CrossOver Wine — handles downloads, DRM, and game launches</div>
          </div>
          <div class="settings-value">
            <div style="display:flex;gap:8px;align-items:center;">
              ${this.wineSteamInstalled
                ? `<span class="badge badge-ok">Installed</span>`
                : this.crossoverAvailable
                ? `<button class="btn btn-primary btn-sm" id="btn-install-steam">Install Steam</button>`
                : `<span class="badge badge-warn">CrossOver Required</span>`}
    ...
    el.querySelector("#btn-install-steam")?.addEventListener("click", async () => {
      this.toast("Installing Steam via CrossOver Wine...");
