/*
 * migrate_backend.c — Metalsharp backend: migration engine stub
 *
 * WHAT  migration engine stub (stub implementation)
 * IMPORTS  server.h, http_server.h
 * EXPORTS  migrate_backend_register_routes
...
void migrate_backend_register_routes(HttpServer* s, Database* db) {
    (void)s;
    (void)db;
}
