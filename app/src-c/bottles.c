...
static const MetalsharpBottlePolicy policies[] = {
    POLICY("m12", "m12", "dxmt_m12", METALSHARP_BOTTLE_ARCH_X86_64, METALSHARP_DXMT_M12_SURFACE_ID,
           METALSHARP_DXMT_M12_MANIFEST_SHA256, m12_artifacts, true),
    POLICY("m11", "m11", "dxmt", METALSHARP_BOTTLE_ARCH_X86_64, METALSHARP_DXMT_LEGACY_SURFACE_ID,
           METALSHARP_DXMT_LEGACY_MANIFEST_SHA256, m11_artifacts, false),
    POLICY("m10", "m10", "dxmt", METALSHARP_BOTTLE_ARCH_X86_64, METALSHARP_DXMT_LEGACY_SURFACE_ID,
           METALSHARP_DXMT_LEGACY_MANIFEST_SHA256, m10_artifacts, false),
    POLICY("m11_32", "m11_32", "dxmt", METALSHARP_BOTTLE_ARCH_I386, METALSHARP_DXMT_I386_SURFACE_ID,
...
bool metalsharp_bottle_policy_valid(const MetalsharpBottlePolicy* policy) {
    ...
    if (has_d3d12 != policy->includes_d3d12 || (policy->includes_d3d12 && strcmp(policy->profile_id, "m12") != 0))
        return false;
    if (strcmp(policy->profile_id, "m12") == 0)
        return strcmp(policy->runtime_lane, "dxmt_m12") == 0 &&
               strcmp(policy->surface_id, METALSHARP_DXMT_M12_SURFACE_ID) == 0 &&
               strcmp(policy->manifest_sha256, METALSHARP_DXMT_M12_MANIFEST_SHA256) == 0;
...
bool metalsharp_bottle_runtime_ready(const char* profile_id, const char* m12_root, size_t m12_root_len) {
    ...
    if (!metalsharp_bottle_policy_valid(policy) || m12_root == NULL || m12_root_len == 0)
        return false;
    if (strcmp(policy->runtime_lane, "dxmt_m12") == 0)
        return metalsharp_m12_dxmt_surface_current(m12_root, m12_root_len);
    /* M10/M11/M9 and 32-bit routes retain their pre-existing dxmt checks.
     * Their deployment path still verifies the actual baseline source files. */
    return true;
}
