...
static inline LONG ref_add(LONG* r) {
    return __sync_add_and_fetch(r, 1);
}
static inline LONG ref_sub(LONG* r) {
    return __sync_sub_and_fetch(r, 1);
}
#include <stdint.h>
#include <windows.h>
#include <winternl.h>
  ...
  public:
    ...
    HRESULT WINAPI LockRect(UINT level, D3DLOCKED_RECT* pLockedRect, const RECT* pRect, DWORD flags) override {
        ...
        if (!m_handle)
            return E_FAIL;
        uint32_t bpp = 4;
        if (m_format == 21 || m_format == 22)
            bpp = 4;
        m_lock_row_pitch = m_width * bpp;
        if (!m_lock_ptr)
            m_lock_ptr = new char[m_lock_row_pitch * m_height];
  ...
  public:
    ...
    HRESULT WINAPI CreateVertexShader(const DWORD* fn, IDirect3DVertexShader9** pp) override {
        ...
        if (fn) {
            ...
            p.bytecode = (uint64_t)(uintptr_t)fn;
            p.bytecode_size = sz;
        }
...
__declspec(dllexport) int WINAPI D3DPERF_BeginEvent(D3DCOLOR color, const WCHAR* name) {
    (void)color;
    (void)name;
    return 0;
}
...
__declspec(dllexport) void WINAPI D3DPERF_SetMarker(D3DCOLOR color, const WCHAR* name) {
    (void)color;
    (void)name;
}

__declspec(dllexport) void WINAPI D3DPERF_SetRegion(D3DCOLOR color, const WCHAR* name) {
    (void)color;
    (void)name;
}
}
