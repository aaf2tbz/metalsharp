 ...
 *
 * Wine ntdll in older MetalSharp runtimes probes for mscompatdb.so during
 * process startup. This replacement is deliberately contract-aware and
 * fail-closed: if the current ntdll does not export the MetalSharp hook
 * contract, the shim returns without touching private Wine syscall tables.
 */
...
static void patch_syscalls(void) {
    ...
    if (!tables || !tables[0].service_table || tables[0].service_limit == 0) {
        trace_line("contract has no service table");
        return;
    }
