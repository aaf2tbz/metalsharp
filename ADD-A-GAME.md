...
|----------|----------|----------|-------------|
| external runtime Wine | `launch_external runtime_wine()` | 64-bit Unity/UE4 D3D11 games | external runtime installed |
...
2. **Is it 64-bit and uses D3D11?** → Try external runtime first. If that fails, try GPTK.
...
CX="/Applications/external runtime.app/Contents/SharedSupport/external runtime"
...
let game_type = match appid {
    // existing entries...
    YOUR_APPID => "external runtime_wine",  // or "dxvk_wine", "gptk_wine", etc.
...
**external runtime games** (simplest):
```rust
fn prepare_your_game(game_dir: &PathBuf, home: &PathBuf, appid: u32) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let wine = external runtime_base.join("lib/wine/x86_64-unix/wine");
    ...
    // Remove DXVK DLLs if present — external runtime has its own D3D stack
...
|----------------|-----------|
| external runtime game (64-bit D3D11) | Ghostrunner (`1139900`) — simplest example |
| external runtime game with DXVK removal | Among Us (`945360`) |
...
**"D3D11-compatible GPU required"** — The game can't find a working D3D11 implementation. Make sure DXVK DLLs are in the game dir (for Wine Devel) or removed (for external runtime).
