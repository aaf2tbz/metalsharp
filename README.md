...
MetalSharp is a macOS app that combines the tools that make Windows gaming on Mac work — CrossOver, Wine, Apple's Game Porting Toolkit, DXVK, MoltenVK, FNA — into a single Electron app with a setup wizard, automatic game configuration, and one-click launch. It figures out the best way to run each game so you don't have to.

## What it does

- **Browses your Steam library** — shows every game you own via the Steam Web API
- **Runs Windows Steam via CrossOver** — installs games through Steam's own interface with full DRM support
- **Auto-configures the runtime** — each game gets the right Wine build, graphics pipeline, and audio bridge
- **Launches with one click** — detects the game engine and picks the best available method, including workarounds for games that don't run natively with D3DMetal/CrossOver

## How games run

MetalSharp combines multiple working methods and adds new ones for games that don't cooperate with a single approach:

**CrossOver + Steam DRM** — CrossOver Wine runs the Windows Steam client natively. Games install through Steam's interface and launch via `steam://run/` for full DRM authentication. This handles most modern 64-bit games (Elden Ring, RE4, Ghostrunner, DREDGE, Sons of the Forest, Among Us).

**D3DMetal performance tuning** — For CrossOver games, MetalSharp applies Apple D3DMetal performance flags (async GPU commit, multithreaded D3D, skip render barriers, NaN safety) to squeeze out better frame rates.
...
| Elden Ring | FromSoftware Engine | CrossOver Wine + MetalFX + Steam DRM | Working |
...
```bash
# 1. Bundle dependencies (compresses CrossOver, GPTK, Mono, DXVK, SteamCMD, SteamSetup.exe)
...
1. **Install** — two buttons. "Install Homebrew" opens Terminal for the brew installer. "Install Dependencies" runs everything else automatically (CrossOver, GPTK, Rosetta 2, Xcode CLI, Mono, DXVK, SteamCMD, Windows Steam). Bundled archives are extracted with a system auth prompt (Touch ID / password); missing deps fall back to `brew install`. Live progress bar and log throughout.
2. **Steam** — set a device name, optionally enter your Steam Web API key, then click "Launch Steam". MetalSharp starts Windows Steam via CrossOver Wine. Log in through the Steam window.
...
| Wine Stable | Standalone Wine for games that don't need CrossOver |
...
|--------|------|------------------|
| `crossover.tar.zst` | ~260 MB | CrossOver.app with Wine 11.0 + Vulkan renderer |
