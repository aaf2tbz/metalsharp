...
```bash
# 1. Bundle dependencies (compresses CrossOver, GPTK, Mono, DXVK, SteamSetup.exe)
...
1. **Install** — two buttons. "Install Homebrew" opens Terminal for the brew installer. "Install Dependencies" runs everything else automatically (CrossOver, GPTK, Rosetta 2, Xcode CLI, Mono, DXVK, Windows Steam). Bundled archives are extracted with a system auth prompt (Touch ID / password); missing deps fall back to `brew install`. Live progress bar and log throughout.
