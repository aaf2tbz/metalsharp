...
- From-source MetalSharp Wine 11.0 build — built from CrossOver's open-source Wine patches with gnutls TLS, custom mscompatdb rules engine, MoltenVK, and freetype
...
- Game setup scripts updated to use MetalSharp Wine instead of CrossOver
- Removed DXVK DLL removal from `prepare_metalsharp_game()` — MetalSharp Wine handles D3D natively
- Removed `crossover.tar.zst` from bundle list — no longer needed

### Removed

- CrossOver dependency — MetalSharp now uses its own from-source Wine build
