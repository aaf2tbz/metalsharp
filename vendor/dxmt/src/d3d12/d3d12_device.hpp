...
struct ID3D12Device11Compat : public ID3D12Device10 {
  ...
  CreateSampler2(const D3D12SamplerDesc2Compat *desc,
...
static constexpr UINT MetalSharpM12TranslationLayerVendorMetalSharp =
    0x4d533132u; /* MS12 */
static constexpr UINT MetalSharpM12TranslationLayerIdDxmtM12 =
    0x44583132u; /* DX12 */
...
enum MetalSharpM12TranslationLayerFeatureFlags : UINT64 {
  ...
  MetalSharpM12TranslationLayerFeatureRenderPassHazardPlanning = 1ull << 9,
  MetalSharpM12TranslationLayerFeaturePresentExecutePlanning = 1ull << 10,
  MetalSharpM12TranslationLayerFeatureReplayExecutePlanning = 1ull << 11,
  MetalSharpM12TranslationLayerFeatureCommandPacketShadowRecording = 1ull << 12,
  ...
  MetalSharpM12TranslationLayerFeatureExpandedNativeReplayCoverage = 1ull << 21,
  MetalSharpM12TranslationLayerFeatureThinPECheckpoint = 1ull << 22,
};
...
public:
  MTLD3D12Device(std::unique_ptr<Device> &&device, IMTLDXGIAdapter *pAdapter);
  ~MTLD3D12Device();
  ...
  void operator delete(void *ptr) { VirtualFree(ptr, 0, MEM_RELEASE); }

  void SetDXGIDevice(IMTLDXGIDevice *dxgi_device) {
    m_dxgi_device = dxgi_device;
  }
  ...
  HRESULT STDMETHODCALLTYPE
  CreateCommandQueue(const D3D12_COMMAND_QUEUE_DESC *desc, REFIID riid,
  ...
  HRESULT STDMETHODCALLTYPE
  CreateCommandList(UINT node_mask, D3D12_COMMAND_LIST_TYPE type,
                    ID3D12CommandAllocator *command_allocator,
                    ID3D12PipelineState *initial_pipeline_state, REFIID riid,
                    void **command_list) override;

  HRESULT STDMETHODCALLTYPE
  CheckFeatureSupport(D3D12_FEATURE feature, void *feature_data,
                      UINT feature_data_size) override;

  HRESULT STDMETHODCALLTYPE
  CreateDescriptorHeap(const D3D12_DESCRIPTOR_HEAP_DESC *desc, REFIID riid,
  ...
  HRESULT STDMETHODCALLTYPE CreateRootSignature(UINT node_mask,
                                                const void *bytecode,
  ...
  CreateConstantBufferView(const D3D12_CONSTANT_BUFFER_VIEW_DESC *desc,
  ...
  void STDMETHODCALLTYPE
  CreateSampler(const D3D12_SAMPLER_DESC *desc,
                D3D12_CPU_DESCRIPTOR_HANDLE descriptor) override;
  ...
  GetResourceAllocationInfo(D3D12_RESOURCE_ALLOCATION_INFO *__ret,
  ...
  HRESULT STDMETHODCALLTYPE
  MakeResident(UINT object_count, ID3D12Pageable *const *objects) override;
  ...
  HRESULT STDMETHODCALLTYPE
  CreateCommandSignature(const D3D12_COMMAND_SIGNATURE_DESC *desc,
  ...
  /*** ID3D12Device2 ***/
  HRESULT STDMETHODCALLTYPE
  CreatePipelineState(const D3D12_PIPELINE_STATE_STREAM_DESC *desc, REFIID riid,
  ...
  HRESULT STDMETHODCALLTYPE CreateCommittedResource1(
      ...
      void **resource) override;
  HRESULT STDMETHODCALLTYPE
  CreateHeap1(const D3D12_HEAP_DESC *desc,
  ...
  HRESULT STDMETHODCALLTYPE CreateMetaCommand(
      ...
      void **meta_command) override;
  HRESULT STDMETHODCALLTYPE
  CreateStateObject(const D3D12_STATE_OBJECT_DESC *desc, REFIID riid,
  ...
  HRESULT STDMETHODCALLTYPE SetBackgroundProcessingMode(
      ...
      HANDLE event, WINBOOL *further_measurements_desired) override;

  /*** ID3D12Device7 ***/
  HRESULT STDMETHODCALLTYPE
  AddToStateObject(const D3D12_STATE_OBJECT_DESC *addition,
  ...
  /*** ID3D12Device9 ***/
  HRESULT STDMETHODCALLTYPE
  CreateShaderCacheSession(const D3D12_SHADER_CACHE_SESSION_DESC *desc,
  ...
  HRESULT STDMETHODCALLTYPE
  CreateCommandQueue1(const D3D12_COMMAND_QUEUE_DESC *desc, REFIID creator_id,
