...
prepare_runtime() {
  ...
  mkdir -p "$BUNDLE_DIR" "$WINE_ROOT" "$WINE_ROOT/lib/dxmt/x86_64-unix" "$WINE_ROOT/lib/dxmt/x86_64-windows"
  ...
  if command -v ditto >/dev/null 2>&1; then
    ditto --noextattr --noqtn "$TMP_DXMT/Graphics/dll/dxmt/x86_64-unix" "$WINE_ROOT/lib/dxmt/x86_64-unix"
    ditto --noextattr --noqtn "$TMP_DXMT/Graphics/dll/dxmt/x86_64-windows" "$WINE_ROOT/lib/dxmt/x86_64-windows"
  else
    cp -R "$TMP_DXMT/Graphics/dll/dxmt/x86_64-unix/." "$WINE_ROOT/lib/dxmt/x86_64-unix/"
    cp -R "$TMP_DXMT/Graphics/dll/dxmt/x86_64-windows/." "$WINE_ROOT/lib/dxmt/x86_64-windows/"
  fi
  cp "$WINE_ROOT/lib/dxmt/x86_64-unix/winemetal.so" "$WINE_ROOT/lib/wine/x86_64-unix/winemetal.so"
...
assert_m12_contract() {
  ...
  if ! cmp -s "$WINE_ROOT/lib/dxmt/x86_64-unix/winemetal.so" "$WINE_ROOT/lib/wine/x86_64-unix/winemetal.so"; then
...
python3 "$ROOT_DIR/tools/d3d12-metal-sdk/scripts/stage-dxmt-runtime.py" \
  ...
  --runtime-dir "$WINE_ROOT/lib/dxmt"
