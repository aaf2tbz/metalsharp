...
bool read_cpu_ticks(CpuTicks& ticks) {
    ...
    kern_return_t kr =
        host_statistics(mach_host_self(), HOST_CPU_LOAD_INFO, reinterpret_cast<host_info_t>(&info), &count);
    if (kr != KERN_SUCCESS)
        return false;
...
#else
uint32_t sysctl_u32(const char*) {
    return static_cast<uint32_t>(std::thread::hardware_concurrency());
}
std::string sysctl_string(const char*) {
    return "unknown";
}
double cpu_usage_sample() {
    return -1.0;
...
double hid_pmu_cpu_temp_c(std::string& source) {
    ...
    for (CFIndex i = 0; i < count; ++i) {
        ...
        const bool looks_like_pmu_temp =
            lowered.find("pmu") != std::string::npos || lowered.find("temp") != std::string::npos;
        if (!looks_like_pmu_temp)
            continue;
...
std::vector<ProcessInfo> session_processes() {
    ...
    while (std::fgets(line, sizeof(line), pipe) && rows.size() < 14) {
        ...
        ProcessInfo p;
        if (!(in >> p.pid >> p.cpu >> p.mem >> p.name))
            continue;
