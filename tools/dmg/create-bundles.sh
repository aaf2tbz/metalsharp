...
build_d3dmetal_native_contract_bundle() {
  local dest="$BUNDLE_DIR/metalsharp-d3dmetal-native-contract.tar.zst"
  if [ -s "$dest" ]; then
    if "$PROJECT_ROOT/tools/bundles/verify-bundles.sh" --bundle-dir "$BUNDLE_DIR" metalsharp-d3dmetal-native-contract.tar.zst >/dev/null 2>&1; then
      echo "SKIP bundle: metalsharp-d3dmetal-native-contract.tar.zst"
      return 0
    fi
    echo "Refreshing stale bundle: metalsharp-d3dmetal-native-contract.tar.zst"
    rm -f "$dest"
  fi
  echo "Building bundle: metalsharp-d3dmetal-native-contract.tar.zst"
  python3 "$PROJECT_ROOT/tools/bundles/create-d3dmetal-native-bundle.py" --out "$BUNDLE_DIR"
}
...
while IFS=$'\t' read -r asset _root _platforms _notes; do
  ...
  if [ "$asset" = "metalsharp-d3dmetal-native-contract.tar.zst" ]; then
    build_d3dmetal_native_contract_bundle
  else
