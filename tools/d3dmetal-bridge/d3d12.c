...
static int init_unix_bridge(void) {
    ...
    if (!ntdll) {
        dbg("[d3d12] FAIL: ntdll not found\n");
        return 0;
    }
    dbg("[d3d12] ntdll=%p\n", ntdll);
    ...
    if (!so_path || !so_path[0]) {
        dbg("[d3d12] FAIL: MS_D3DMETAL_UNIXLIB not set\n");
        return 0;
    }
    dbg("[d3d12] so_path='%s'\n", so_path);
    ...
    if (len <= 0) {
        dbg("[d3d12] FAIL: MultiByteToWideChar returned %d err=%lu\n", len, GetLastError());
        return 0;
    }
