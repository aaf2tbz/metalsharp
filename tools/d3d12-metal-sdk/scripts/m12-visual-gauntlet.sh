...
for key in ["background_preserved", "triangle_changed_pixels", "indexed_geometry_verified", "compute_buffer_draw_verified", "compute_texture_draw_verified", "indexed_texture_verified", "depth_verified", "render_changed_from_clear"]:
...
add("compute_texture_pixel", headless.get("readback", {}).get("compute_texture") == [34, 68, 204, 255], headless.get("readback", {}).get("compute_texture"), [34, 68, 204, 255], str(headless_path))
...
summary = {
    ...
    "residual_risks": [
        ...
        "Compute writes are verified through UAV readback, compute-buffer-then-draw, and compute-texture-then-sample/draw pixel paths.",
