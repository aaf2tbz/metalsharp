...
Options:
  ...
  --expect-dxgi-sha SHA      Forward strict dxgi.dll hash gate to bounded launch.
  --expect-dxgi-dxmt-sha SHA Forward strict dxgi_dxmt.dll hash gate to bounded launch.
  --expect-winemetal-dll-sha SHA Forward strict winemetal.dll hash gate to bounded launch.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --workers|--async-compile|--typed-stage-in|--force-source-compile|--expect-d3d12-sha|--expect-dxgi-sha|--expect-dxgi-dxmt-sha|--expect-winemetal-dll-sha|--expect-winemetal-so-sha) BOUNDED_EXTRA+=("$1" "$2"); shift 2 ;;
    -h|--help) usage; exit 0 ;;
