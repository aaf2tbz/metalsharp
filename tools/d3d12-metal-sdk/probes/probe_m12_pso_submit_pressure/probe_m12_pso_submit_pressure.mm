...
static enum WMTPixelFormat pixelFormat(NSString *name) {
  if (!name || [name length] == 0) return WMTPixelFormatInvalid;
  NSString *n = [name lowercaseString];
  if ([n isEqualToString:@"invalid"] || [n isEqualToString:@"unknown"]) return WMTPixelFormatInvalid;
  if ([n isEqualToString:@"r8unorm"] || [n isEqualToString:@"r8_unorm"]) return WMTPixelFormatR8Unorm;
  if ([n isEqualToString:@"rg8unorm"] || [n isEqualToString:@"r8g8_unorm"]) return WMTPixelFormatRG8Unorm;
  if ([n isEqualToString:@"r16unorm"] || [n isEqualToString:@"r16_unorm"]) return WMTPixelFormatR16Unorm;
  if ([n isEqualToString:@"r16float"] || [n isEqualToString:@"r16_float"]) return WMTPixelFormatR16Float;
  if ([n isEqualToString:@"rg16unorm"] || [n isEqualToString:@"r16g16_unorm"]) return WMTPixelFormatRG16Unorm;
  if ([n isEqualToString:@"rg16float"] || [n isEqualToString:@"r16g16_float"]) return WMTPixelFormatRG16Float;
  if ([n isEqualToString:@"rgba8unorm"] || [n isEqualToString:@"r8g8b8a8_unorm"]) return WMTPixelFormatRGBA8Unorm;
  if ([n isEqualToString:@"rgba8unorm_srgb"] || [n isEqualToString:@"r8g8b8a8_unorm_srgb"]) return WMTPixelFormatRGBA8Unorm_sRGB;
  if ([n isEqualToString:@"bgra8unorm"] || [n isEqualToString:@"b8g8r8a8_unorm"]) return WMTPixelFormatBGRA8Unorm;
  if ([n isEqualToString:@"bgra8unorm_srgb"] || [n isEqualToString:@"b8g8r8a8_unorm_srgb"]) return WMTPixelFormatBGRA8Unorm_sRGB;
  if ([n isEqualToString:@"rgba16unorm"] || [n isEqualToString:@"r16g16b16a16_unorm"]) return WMTPixelFormatRGBA16Unorm;
  if ([n isEqualToString:@"rgba16float"] || [n isEqualToString:@"r16g16b16a16_float"]) return WMTPixelFormatRGBA16Float;
  if ([n isEqualToString:@"rgb10a2unorm"] || [n isEqualToString:@"r10g10b10a2_unorm"]) return WMTPixelFormatRGB10A2Unorm;
  if ([n isEqualToString:@"rg11b10float"] || [n isEqualToString:@"r11g11b10_float"]) return WMTPixelFormatRG11B10Float;
  if ([n isEqualToString:@"r32float"] || [n isEqualToString:@"r32_float"]) return WMTPixelFormatR32Float;
  if ([n isEqualToString:@"r32uint"] || [n isEqualToString:@"r32_uint"]) return WMTPixelFormatR32Uint;
  if ([n isEqualToString:@"rgba32float"] || [n isEqualToString:@"r32g32b32a32_float"]) return WMTPixelFormatRGBA32Float;
  if ([n isEqualToString:@"depth16unorm"] || [n isEqualToString:@"d16_unorm"]) return WMTPixelFormatDepth16Unorm;
  if ([n isEqualToString:@"depth32float"] || [n isEqualToString:@"d32_float"]) return WMTPixelFormatDepth32Float;
  if ([n isEqualToString:@"depth24unorm_stencil8"] || [n isEqualToString:@"d24_unorm_s8_uint"]) return WMTPixelFormatDepth24Unorm_Stencil8;
  if ([n isEqualToString:@"depth32float_stencil8"] || [n isEqualToString:@"d32_float_s8x24_uint"]) return WMTPixelFormatDepth32Float_Stencil8;
...
static uint32_t formatByteSize(uint32_t format) {
  switch (format) {
    ...
    case WMTAttributeFormatChar2: return 2;
    case WMTAttributeFormatUChar3:
    case WMTAttributeFormatChar3: return 3;
    ...
    case WMTAttributeFormatUInt: return 4;
    ...
    case WMTAttributeFormatUInt2: return 8;
    case WMTAttributeFormatFloat3:
    case WMTAttributeFormatInt3:
    case WMTAttributeFormatUInt3: return 12;
    case WMTAttributeFormatFloat4:
    case WMTAttributeFormatInt4:
    case WMTAttributeFormatUInt4: return 16;
    default: return 16;
  }
}
...
static id<MTLFunction> createShaderFunction(id<MTLDevice> device, NSDictionary *shader, uint32_t stage,
  ...
  if (rc != 0 || result.abi_version != M12CORE_ABI_VERSION ||
      result.status != M12CORE_SHADER_FUNCTION_STATUS_OK || !result.function_handle) {
    fprintf(stderr, "m12core shader function failed hash=%s stage=%u rc=%d status=%u path=%s\n",
            [stringValue(shader, @"hash", @"") UTF8String], stage, rc, result.status, path ? [path UTF8String] : "");
...
static bool createComputePipeline(id<MTLDevice> device, NSDictionary *pipeline, uint32_t worker, uint32_t iteration,
  ...
  id<MTLFunction> compute = createShaderFunction(device, dictValue(pipeline, @"shader"), M12CORE_SHADER_STAGE_COMPUTE, counters.shader_failed);
  ...
  if (rc == 0 && result.abi_version == M12CORE_ABI_VERSION && result.status == M12CORE_PIPELINE_CREATE_STATUS_OK && result.pipeline_handle) {
    if (result.cache_hit) counters.pso_cache_hits.fetch_add(1);
  ...
  if (result.error_handle) {
    ...
    fprintf(stderr, "compute PSO failed name=%s status=%u error=%s\n",
            [stringValue(pipeline, @"name", @"") UTF8String], result.status, [[error localizedDescription] UTF8String]);
  }
  return false;
}

static void fillVertexDescriptor(NSDictionary *pipeline, WMTVertexDescriptor &vd, uint32_t strides[WMT_MAX_VERTEX_BUFFER_LAYOUTS]) {
  ...
  for (NSDictionary *element in elements) {
    ...
    if ([element[@"system_value"] respondsToSelector:@selector(boolValue)] && [element[@"system_value"] boolValue]) continue;
    uint32_t attr = [element[@"register"] respondsToSelector:@selector(unsignedIntValue)] ? [element[@"register"] unsignedIntValue] : UINT32_MAX;
    uint32_t fmt = [element[@"metal_format"] respondsToSelector:@selector(unsignedIntValue)] ? [element[@"metal_format"] unsignedIntValue] : 0;
    uint32_t slot = [element[@"table_index"] respondsToSelector:@selector(unsignedIntValue)] ? [element[@"table_index"] unsignedIntValue] : 0;
    uint32_t offset = [element[@"offset"] respondsToSelector:@selector(unsignedIntValue)] ? [element[@"offset"] unsignedIntValue] : 0;
    if (attr >= WMT_MAX_VERTEX_ATTRIBUTES || slot >= WMT_MAX_VERTEX_BUFFER_LAYOUTS || fmt == WMTAttributeFormatInvalid) continue;
    vd.attributes[attr].format = (WMTAttributeFormat)fmt;
    vd.attributes[attr].offset = offset;
    vd.attributes[attr].buffer_index = slot;
    if (attr + 1 > vd.attribute_count) vd.attribute_count = attr + 1;
    if (slot + 1 > vd.layout_count) vd.layout_count = slot + 1;
    ...
    vd.layouts[slot].step_function = [klass isEqualToString:@"per_instance"] ? WMTVertexStepFunctionPerInstance : WMTVertexStepFunctionPerVertex;
    vd.layouts[slot].step_rate = [element[@"step_rate"] respondsToSelector:@selector(unsignedIntValue)] ? [element[@"step_rate"] unsignedIntValue] : 1;
  }
  for (uint32_t i = 0; i < vd.layout_count; i++) {
    vd.layouts[i].stride = strides[i] ? strides[i] : 16;
    if (vd.layouts[i].step_rate == 0) vd.layouts[i].step_rate = 1;
  }
}
...
static bool createRenderPipeline(id<MTLDevice> device, NSDictionary *pipeline, uint32_t worker, uint32_t iteration,
  ...
  id<MTLFunction> vertex = createShaderFunction(device, dictValue(pipeline, @"vertex"), M12CORE_SHADER_STAGE_VERTEX, counters.shader_failed);
  ...
  if (fragmentDict && !fragment) return false;
  ...
  for (NSUInteger i = 0; i < 8; i++) {
    NSString *fmt = (formats && i < [formats count] && [formats[i] isKindOfClass:[NSString class]]) ? formats[i] : @"invalid";
  ...
  info.raster_sample_count = [pipeline[@"sample_count"] respondsToSelector:@selector(unsignedCharValue)] ? [pipeline[@"sample_count"] unsignedCharValue] : 1;
  if (info.raster_sample_count == 0) info.raster_sample_count = 1;
  ...
  if (result.error_handle) {
    ...
    fprintf(stderr, "render PSO failed name=%s rc=%d status=%u\n", [stringValue(pipeline, @"name", @"") UTF8String], rc, result.status);
  }
  return false;
}
...
static id<MTLTexture> renderTextureForFormat(id<MTLDevice> device, RenderTargetCache &cache,
                                             uint32_t format, uint32_t sampleCount) {
...
static bool encodeOffscreenRenderPass(id<MTLDevice> device, id<MTLCommandBuffer> cb,
                                      RenderTargetCache &cache, RenderPsoPool *renderPool,
                                      Counters &counters) {
  ...
  if (record.stencilFormat != WMTPixelFormatInvalid) {
    id<MTLTexture> stencil = depthStencil ?: renderTextureForFormat(device, cache, record.stencilFormat, record.sampleCount);
    if (stencil) {
...
static void runCommandBufferPressure(id<MTLDevice> device, uint32_t queueDepth, uint32_t durationMs,
                                     ...
                                     RenderPsoPool *renderPool, bool encodeRenderPasses,
                                     bool encodeDrawablePresents, bool offscreenWindow,
                                     bool displaySync, uint32_t drawableWindowCount,
                                     uint64_t minCommandBuffers,
                                     uint32_t drawableChurnInterval) {
  ...
  if (encodeDrawablePresents) {
    ...
    for (uint32_t i = 0; i < count; i++) {
      MSPressureDrawableTarget *target = createDrawableTarget(device, 128 + 8 * i, 96 + 4 * i, offscreenWindow, displaySync);
      if (target)
  ...
  for (uint32_t i = 0; i < depth; i++) slots.push_back(std::make_unique<CompletionSlot>());
  uint64_t serial = 0;
  uint64_t index = 0;
  ...
  while (Clock::now() < deadline) {
    ...
    @autoreleasepool {
      ...
      if (encodeDrawablePresents) {
        MSPressureDrawableTarget *target = [drawableTargets count] ? drawableTargets[(NSUInteger)(index % [drawableTargets count])] : nil;
...
static void usage(const char *argv0) {
  fprintf(stderr, "usage: %s --manifest manifest.json --output result.json [--mode baseline|storm|pso-cmdb|pso-render-cmdb|pso-drawable-cmdb] [--pso-workers N] [--iterations N] [--queue-depth N] [--duration-ms N] [--slot-wait-ms N] [--max-pipelines N] [--min-command-buffers N] [--drawable-window-count N] [--drawable-churn-interval N] [--salt-cache-keys] [--onscreen-window] [--display-sync]\n", argv0);
}
...
int main(int argc, const char **argv) {
  @autoreleasepool {
    ...
    for (int i = 1; i < argc; i++) {
      if (!strcmp(argv[i], "--manifest") && i + 1 < argc) manifestPath = [NSString stringWithUTF8String:argv[++i]];
      else if (!strcmp(argv[i], "--output") && i + 1 < argc) outputPath = [NSString stringWithUTF8String:argv[++i]];
      else if (!strcmp(argv[i], "--mode") && i + 1 < argc) mode = argv[++i];
      else if (!strcmp(argv[i], "--pso-workers") && i + 1 < argc) psoWorkers = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--iterations") && i + 1 < argc) iterations = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--queue-depth") && i + 1 < argc) queueDepth = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--duration-ms") && i + 1 < argc) durationMs = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--slot-wait-ms") && i + 1 < argc) slotWaitMs = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--max-pipelines") && i + 1 < argc) maxPipelines = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--min-command-buffers") && i + 1 < argc) minCommandBuffers = strtoull(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--drawable-window-count") && i + 1 < argc) drawableWindowCount = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--drawable-churn-interval") && i + 1 < argc) drawableChurnInterval = (uint32_t)strtoul(argv[++i], nullptr, 10);
      else if (!strcmp(argv[i], "--salt-cache-keys")) saltCacheKeys = true;
      else if (!strcmp(argv[i], "--onscreen-window")) offscreenWindow = false;
      else if (!strcmp(argv[i], "--display-sync")) displaySync = true;
      else { usage(argv[0]); return 2; }
    }
    if (!manifestPath || !outputPath) { usage(argv[0]); return 2; }
    if (psoWorkers == 0) psoWorkers = 1;
    ...
    if (maxPipelines > 0 && count > maxPipelines) count = maxPipelines;
        ...
        @autoreleasepool {
          while (true) {
            ...
            if ([type isEqualToString:@"render"])
              ok = createRenderPipeline(device, pipeline, w, iter, saltCacheKeys, counters, encodeRenderPasses ? &renderPool : nullptr);
            else if ([type isEqualToString:@"compute"])
              ok = createComputePipeline(device, pipeline, w, iter, saltCacheKeys, counters);
            else
              fprintf(stderr, "skipping unsupported pipeline type=%s name=%s\n", [type UTF8String], [stringValue(pipeline, @"name", @"") UTF8String]);
            if (ok) counters.pso_ok.fetch_add(1);
            else counters.pso_failed.fetch_add(1);
          }
        }
        active.fetch_sub(1);
      });
    ...
    if (mode == "pso-cmdb" || mode == "pso-render-cmdb" || mode == "pso-drawable-cmdb")
      runCommandBufferPressure(device, queueDepth, durationMs, slotWaitMs, active, counters, &renderPool,
                               encodeRenderPasses, encodeDrawablePresents, offscreenWindow,
                               displaySync, drawableWindowCount, minCommandBuffers,
                               drawableChurnInterval);
    ...
    NSDictionary *result = @{
      ...
      @"ok": @(counters.pso_failed.load() == 0 && counters.slot_timeouts.load() == 0 && counters.command_buffers_error_status.load() == 0 && counters.render_passes_failed.load() == 0 && counters.drawable_passes_failed.load() == 0),
