...
static HRESULT compile_shader(const char* hlsl, const char* entry, ID3DBlob** out, std::string& errors) {
    ...
    HRESULT hr = g_compile ? g_compile(hlsl, std::strlen(hlsl), "probe_barriers_render_pass.hlsl", nullptr, nullptr,
...
int main() {
    ...
    std::printf("{\n");
    std::printf("  \"schema\": \"metalsharp.d3d12-metal.probe-barriers-render-pass.v1\",\n");
    std::printf("  \"profile\": \"%s\",\n", json_escape(profile).c_str());
    std::printf("  \"pass\": %s,\n", pass ? "true" : "false");
    std::printf("  \"coverage\": {\n");
    std::printf("    \"render_target_to_shader_resource_status\": true,\n");
