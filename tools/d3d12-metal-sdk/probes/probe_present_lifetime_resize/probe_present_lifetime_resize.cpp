...
static CaseResult check_present_submission_retention(const std::string& swapchain) {
    ...
    r.detail = r.pass ? "present command buffers retain source backbuffer/resource, source/destination textures, drawable, and drawable texture until completion"
...
static CaseResult check_present_hot_path_avoids_heavy_retention_temporaries(const std::string& swapchain) {
    ...
    r.pass = !present1.empty() &&
             !contains(present1, "D3D12MetalSubmissionReferences") &&
    ...
    r.detail = r.pass ? "Present1 delegates present retention out of the hot path instead of constructing heavyweight submission-reference temporaries inline"
...
static CaseResult check_single_present_ownership(const std::string& swapchain) {
    ...
    r.pass = contains(swapchain, "if (!native_present_executed") &&
             ...
             contains(swapchain, "native_present_executed = true") &&
             contains(swapchain, "raw-post-native") &&
             contains(swapchain, "presenter-fallback");
    r.detail = r.pass ? "presenter/raw/native ownership paths are mutually classified and raw presentDrawable is skipped after native execution"
...
static CaseResult check_resize_cage(const std::string& swapchain) {
    ...
    r.pass = resize_pos != std::string::npos && drain_pos != std::string::npos &&
             release_pos != std::string::npos && drain_pos < release_pos;
    r.detail = r.pass ? "ResizeBuffers drains in-flight present command buffers before releasing/recreating old backbuffers"
...
static CaseResult check_completion_slots_release_after_done(const std::string& swapchain) {
    ...
    r.pass = contains(swapchain, "WaitForPresentCommandBufferSlot") &&
             ...
             contains(swapchain, "TrackPresentCommandBuffer(") &&
             contains(swapchain, "ArmD3D12MetalCompletionSlot(");
    r.detail = r.pass ? "present inflight slots wait/observe completion before Reset clears retained submission references"
