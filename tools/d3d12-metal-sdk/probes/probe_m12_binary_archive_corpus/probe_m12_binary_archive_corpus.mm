...
static MTLVertexDescriptor* makeVertexDescriptor(NSDictionary* pipeline) {
    ...
    for (NSDictionary* element in elements) {
        ...
        if ([systemValue respondsToSelector:@selector(boolValue)] && [systemValue boolValue])
            continue;
        NSUInteger attr = [element[@"register"] respondsToSelector:@selector(unsignedIntegerValue)]
                              ? [element[@"register"] unsignedIntegerValue]
                              : NSUIntegerMax;
        NSUInteger slot = [element[@"table_index"] respondsToSelector:@selector(unsignedIntegerValue)]
                              ? [element[@"table_index"] unsignedIntegerValue]
                              : 0;
        NSUInteger offset = [element[@"offset"] respondsToSelector:@selector(unsignedIntegerValue)]
                                ? [element[@"offset"] unsignedIntegerValue]
                                : 0;
        NSUInteger fmt = [element[@"metal_format"] respondsToSelector:@selector(unsignedIntegerValue)]
                             ? [element[@"metal_format"] unsignedIntegerValue]
                             : MTLVertexFormatInvalid;
        if (attr >= 31 || slot >= 31 || fmt == MTLVertexFormatInvalid)
            continue;
        ...
        vd.layouts[slot].stepFunction =
            [klass isEqualToString:@"per_instance"] ? MTLVertexStepFunctionPerInstance : MTLVertexStepFunctionPerVertex;
        NSUInteger stepRate = [element[@"step_rate"] respondsToSelector:@selector(unsignedIntegerValue)]
                                  ? [element[@"step_rate"] unsignedIntegerValue]
                                  : 1;
...
static id<MTLLibrary> makeLibrary(id<MTLDevice> device, NSDictionary* shader, NSMutableString* errorOut) {
    ...
    if ([kind isEqualToString:@"metallib"]) {
        ...
        dispatch_data_t dispatchData =
            dispatch_data_create([data bytes], [data length], NULL, DISPATCH_DATA_DESTRUCTOR_DEFAULT);
        id<MTLLibrary> lib = [device newLibraryWithData:dispatchData error:&error];
        if (!lib)
            [errorOut appendFormat:@"newLibraryWithData failed: %@", errorString(error)];
        return lib;
    }
    ...
    if (!lib)
        [errorOut appendFormat:@"newLibraryWithSource failed: %@", errorString(error)];
    return lib;
}
...
static id<MTLFunction> makeFunction(id<MTLDevice> device, NSDictionary* shader, NSMutableString* errorOut) {
    ...
    if (!lib)
        return nil;
    NSArray* names = @[ stringValue(shader, @"function", @""), @"main", @"cs_main", @"vs_main", @"ps_main" ];
    NSMutableSet* seen = [NSMutableSet set];
    for (NSString* name in names) {
        if (![name isKindOfClass:[NSString class]] || [name length] == 0 || [seen containsObject:name])
            continue;
        [seen addObject:name];
...
static MTLRenderPipelineDescriptor* makeRenderDescriptor(id<MTLDevice> device, NSDictionary* pipeline,
                                                         NSMutableString* errorOut) {
    ...
    NSUInteger sampleCount = [pipeline[@"sample_count"] respondsToSelector:@selector(unsignedIntegerValue)]
                                 ? [pipeline[@"sample_count"] unsignedIntegerValue]
                                 : 1;
    ...
    if (!hasColor && desc.depthAttachmentPixelFormat == MTLPixelFormatInvalid &&
...
static MTLComputePipelineDescriptor* makeComputeDescriptor(id<MTLDevice> device, NSDictionary* pipeline,
                                                           NSMutableString* errorOut) {
...
static id<MTLBinaryArchive> newArchive(id<MTLDevice> device, NSString* path, NSMutableString* errorOut) {
    ...
    if (!archive)
        [errorOut appendFormat:@"newBinaryArchive failed path=%@: %@", path ?: @"", errorString(error)];
    return archive;
}

static BOOL createPipeline(id<MTLDevice> device, NSDictionary* candidate, id<MTLBinaryArchive> lookupArchive,
                           BOOL strict, NSMutableString* errorOut) {
    ...
    if ([type isEqualToString:@"compute"]) {
        ...
        if (!desc)
            return NO;
        if (lookupArchive)
            desc.binaryArchives = @[ lookupArchive ];
...
static BOOL addFunctionsToArchive(id<MTLDevice> device, NSDictionary* candidate, id<MTLBinaryArchive> archive,
                                  NSMutableString* errorOut) {
    NSDictionary* pipeline = dictValue(candidate, @"pipeline");
    NSString* type = stringValue(candidate, @"type", @"");
    NSError* error = nil;
    if ([type isEqualToString:@"compute"]) {
        ...
        if (!ok)
            [errorOut appendFormat:@"addComputePipelineFunctions failed: %@", errorString(error)];
        return ok;
    }
    if ([type isEqualToString:@"render"]) {
        MTLRenderPipelineDescriptor* desc = makeRenderDescriptor(device, pipeline, errorOut);
        ...
        if (!ok)
            [errorOut appendFormat:@"addRenderPipelineFunctions failed: %@", errorString(error)];
        return ok;
    }
    [errorOut appendFormat:@"unsupported type %@", type];
    return NO;
...
int main(int argc, const char** argv) {
    @autoreleasepool {
        ...
        id<MTLBinaryArchive> reloaded =
            (serializeOK && archiveBytes > 0) ? newArchive(device, archivePath, reloadError) : nil;
