...
static HRESULT init_context(Context& ctx) {
    ...
    if (!ctx.create_device || !ctx.compile || !ctx.serialize)
        return E_FAIL;
    HRESULT hr =
        ctx.create_device(nullptr, D3D_FEATURE_LEVEL_11_0, IID_D3D12DeviceProbe, reinterpret_cast<void**>(&ctx.device));
    D3D12_COMMAND_QUEUE_DESC qdesc = {};
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateCommandQueue(&qdesc, IID_PPV_ARGS(&ctx.queue));
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&ctx.allocator));
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, ctx.allocator, nullptr,
    ...
    if (SUCCEEDED(hr) && !ctx.event_handle)
        hr = E_FAIL;

    const char* hlsl = "struct VSIn{float4 pos:POSITION;float4 color:COLOR0;};"
    ...
    if (SUCCEEDED(hr))
        hr = ctx.compile(hlsl, std::strlen(hlsl), "execute_indirect_draw", nullptr, nullptr, "vs_main", "vs_5_0", 0, 0,
                         &vs, &errors);
    safe_release(errors);
    if (SUCCEEDED(hr))
        hr = ctx.compile(hlsl, std::strlen(hlsl), "execute_indirect_draw", nullptr, nullptr, "ps_main", "ps_5_0", 0, 0,
                         &ps, &errors);
    safe_release(errors);
    D3D12_ROOT_SIGNATURE_DESC root_desc = {};
    root_desc.Flags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT;
    if (SUCCEEDED(hr))
        hr = ctx.serialize(&root_desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, &errors);
    safe_release(errors);
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(),
                                             IID_PPV_ARGS(&ctx.root));
    ...
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateGraphicsPipelineState(&pso_desc, IID_PPV_ARGS(&ctx.pso));
    ...
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &vb_desc,
                                                 D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&ctx.vb));
    if (SUCCEEDED(hr))
        upload_data(ctx.vb, vertices, sizeof(vertices), hr);
    auto ib_desc = buffer_desc(sizeof(indices));
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &ib_desc,
                                                 D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&ctx.ib));
    if (SUCCEEDED(hr))
        upload_data(ctx.ib, indices, sizeof(indices), hr);
    if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateCommandSignature(&draw_desc, nullptr, IID_PPV_ARGS(&ctx.draw_sig));
    ...
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateCommandSignature(&indexed_desc, nullptr, IID_PPV_ARGS(&ctx.draw_indexed_sig));
...
static void destroy_context(Context& ctx) {
    safe_release(ctx.ib);
    safe_release(ctx.vb);
    safe_release(ctx.draw_indexed_sig);
    safe_release(ctx.draw_sig);
    safe_release(ctx.pso);
    safe_release(ctx.root);
    safe_release(ctx.fence);
    safe_release(ctx.list);
    safe_release(ctx.allocator);
    safe_release(ctx.queue);
    safe_release(ctx.device);
    if (ctx.event_handle)
        CloseHandle(ctx.event_handle);
    if (ctx.d3dcompiler)
        FreeLibrary(ctx.d3dcompiler);
    if (ctx.d3d12)
...
static CaseResult run_case(Context& ctx, const char* name, ID3D12CommandSignature* sig, UINT max_command_count,
    ...
    ID3D12Resource* args_upload = nullptr;
    ID3D12Heap* args_heap = nullptr;
    ID3D12Resource* count = nullptr;
    ID3D12Resource* count_upload = nullptr;
    ID3D12Heap* count_heap = nullptr;
    HRESULT hr = S_OK;
    ...
    clear.Color[3] = 1.0f;
    hr = ctx.device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &rt_desc,
                                             D3D12_RESOURCE_STATE_RENDER_TARGET, &clear, IID_PPV_ARGS(&render_target));
    D3D12_DESCRIPTOR_HEAP_DESC rtv_desc = {};
    rtv_desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;
    rtv_desc.NumDescriptors = 1;
    if (SUCCEEDED(hr))
        hr = ctx.device->CreateDescriptorHeap(&rtv_desc, IID_PPV_ARGS(&rtv_heap));
    if (SUCCEEDED(hr))
        ctx.device->CreateRenderTargetView(render_target, nullptr, rtv_heap->GetCPUDescriptorHandleForHeapStart());
    ...
    if (SUCCEEDED(hr) && default_heap_args) {
        if (placed_heap_args) {
            ...
            if (SUCCEEDED(hr))
                hr = ctx.device->CreatePlacedResource(args_heap, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT, &arg_desc,
                                                      D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&args));
        } else {
            hr = ctx.device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &arg_desc,
                                                     D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&args));
        }
        if (SUCCEEDED(hr))
            hr = ctx.device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &arg_desc,
                                                     D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
                                                     IID_PPV_ARGS(&args_upload));
        if (SUCCEEDED(hr))
            upload_data(args_upload, initial_args, args_size, hr);
    } else if (SUCCEEDED(hr)) {
        hr = ctx.device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &arg_desc,
                                                 D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&args));
        if (SUCCEEDED(hr))
            upload_data(args, initial_args, args_size, hr);
    }
    if (SUCCEEDED(hr) && count_value) {
        ...
        if (default_heap_count) {
            if (placed_heap_count) {
                ...
                if (SUCCEEDED(hr))
                    hr = ctx.device->CreatePlacedResource(count_heap, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT,
                                                          &count_desc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                                          IID_PPV_ARGS(&count));
            } else {
                hr = ctx.device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &count_desc,
                                                         D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&count));
            }
            if (SUCCEEDED(hr))
                hr = ctx.device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &count_desc,
                                                         D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
                                                         IID_PPV_ARGS(&count_upload));
            if (SUCCEEDED(hr))
                upload_data(count_upload, count_value, sizeof(uint32_t), hr);
        } else {
            hr = ctx.device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &count_desc,
                                                     D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&count));
            if (SUCCEEDED(hr))
                upload_data(count, count_value, sizeof(uint32_t), hr);
        }
    }
    if (SUCCEEDED(hr))
        hr = ctx.allocator->Reset();
    if (SUCCEEDED(hr))
        hr = ctx.list->Reset(ctx.allocator, ctx.pso);
    if (SUCCEEDED(hr)) {
        if (args_upload) {
        ...
        ctx.list->RSSetScissorRects(1, &sc);
        ctx.list->OMSetRenderTargets(1, &rtv, FALSE, nullptr);
        ctx.list->ClearRenderTargetView(rtv, black, 0, nullptr);
        ctx.list->SetGraphicsRootSignature(ctx.root);
        ctx.list->SetPipelineState(ctx.pso);
        ctx.list->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        ctx.list->IASetVertexBuffers(0, 1, &ctx.vbv);
        ctx.list->IASetIndexBuffer(&ctx.ibv);
        ctx.list->ExecuteIndirect(sig, max_command_count, args, 0, count, 0);
        hr = ctx.list->Close();
    }
    if (SUCCEEDED(hr) && mutation_args && mutation_size)
        upload_data(args, mutation_args, mutation_size, hr);
    if (SUCCEEDED(hr)) {
        ID3D12CommandList* lists[] = {ctx.list};
        ctx.queue->ExecuteCommandLists(1, lists);
        ctx.fence_value++;
        hr = ctx.queue->Signal(ctx.fence, ctx.fence_value);
        if (SUCCEEDED(hr) && !wait_for_fence(ctx.fence, ctx.fence_value, ctx.event_handle))
            hr = E_FAIL;
    }
    if (SUCCEEDED(hr))
        read_render_target(ctx.device, ctx.queue, ctx.allocator, ctx.list, ctx.fence, ctx.event_handle, ctx.fence_value,
                           render_target, result.observed, hr);
    ...
    if (result.pass && (default_heap_args || default_heap_count))
        ...
        result.detail = result.pass ? "ExecuteIndirect draw replay matched expected color"
                                    : "ExecuteIndirect draw replay color mismatch";
    safe_release(count_upload);
    safe_release(count);
    safe_release(count_heap);
    safe_release(args_upload);
    safe_release(args);
    safe_release(args_heap);
...
int main() {
    ...
    if (SUCCEEDED(init_hr)) {
        ...
        cases.push_back(run_case(ctx, "execute_indirect_draw", ctx.draw_sig, 1, &draw_red, sizeof(draw_red), nullptr, 0,
                                 nullptr, 255, 0, 0));
        cases.push_back(run_case(ctx, "execute_indirect_draw_indexed", ctx.draw_indexed_sig, 1, &draw_indexed_green,
                                 sizeof(draw_indexed_green), nullptr, 0, nullptr, 0, 255, 0));
        cases.push_back(run_case(ctx, "execute_indirect_count_zero", ctx.draw_sig, 1, &draw_red, sizeof(draw_red),
                                 nullptr, 0, &zero, 0, 0, 0));
        cases.push_back(run_case(ctx, "execute_indirect_count_n", ctx.draw_sig, 2, draw_pair, sizeof(draw_pair),
                                 nullptr, 0, &two, 0, 0, 255));
        cases.push_back(run_case(ctx, "execute_indirect_args_mutated_before_execute", ctx.draw_sig, 1, &draw_red,
                                 sizeof(draw_red), &draw_blue, sizeof(draw_blue), &one, 0, 0, 255));
        cases.push_back(run_case(ctx, "execute_indirect_gpu_argument_buffer_materialized", ctx.draw_sig, 1, &draw_red,
                                 sizeof(draw_red), nullptr, 0, nullptr, 255, 0, 0, true, false));
        cases.push_back(run_case(ctx, "execute_indirect_gpu_count_buffer_materialized", ctx.draw_sig, 1, &draw_red,
                                 sizeof(draw_red), nullptr, 0, &one, 255, 0, 0, false, true));
        cases.push_back(run_case(ctx, "execute_indirect_placed_gpu_argument_buffer_materialized", ctx.draw_sig, 1,
                                 &draw_red, sizeof(draw_red), nullptr, 0, nullptr, 255, 0, 0, true, false, true,
                                 false));
        cases.push_back(run_case(ctx, "execute_indirect_placed_gpu_count_buffer_materialized", ctx.draw_sig, 1,
                                 &draw_red, sizeof(draw_red), nullptr, 0, &one, 255, 0, 0, false, true, false, true));
    } else {
        cases.push_back(
            {"setup", false, init_hr, {0, 0, 0, 0}, "failed to initialize D3D12 ExecuteIndirect draw probe"});
    }
    bool pass = true;
