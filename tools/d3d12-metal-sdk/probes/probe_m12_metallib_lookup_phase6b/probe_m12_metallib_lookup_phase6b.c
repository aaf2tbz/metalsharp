...
static const char *json_bool(uint32_t v) { return v ? "true" : "false"; }
...
int main(int argc, char **argv) {
  ...
  fprintf(f,
          ...
          "}\n",
          rc, lookup.abi_version, hash, json_bool(lookup.force_source_compile),
          json_bool(lookup.metallib_exists), json_bool(lookup.metallib_available),
          lookup.paths.metallib_path, lookup.paths.metallib_error_path);
  fclose(f);
  return rc == 0 ? 0 : 1;
}
