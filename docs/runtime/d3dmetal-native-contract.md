...
| Wine DLL overrides | `WINEDLLOVERRIDES` | `d3d10,d3d10_1,d3d10core,d3d11,d3d12,dxgi,nvapi64,nvngx,nvngx-on-metalfx=n,b;gameoverlayrenderer,gameoverlayrenderer64=d` |
...
- `D3D11CreateDevice(NULL)` returns `hr=0x00000000` when GPTK4 route DLLs are staged app-locally and selected with native-first `WINEDLLOVERRIDES` (`=n,b`). `WINEDLLPATH` alone is not accepted as proof because Wine 11.5 can still fall back to stock builtins.
