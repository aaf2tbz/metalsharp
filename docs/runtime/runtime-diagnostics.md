...
- `vulkan` — filesystem-only Vulkan-family doctor. It checks canonical `runtime/wine/lib/dxvk`, `runtime/wine/lib/vkd3d`, required MoltenVK/VKD3D sidecars, MoltenVK ICD JSON paths, and DXVK state-cache path writability inferred from permission bits without launching Wine/Vulkan or creating probe files.
