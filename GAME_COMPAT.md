...
### Resident Evil 4 (2050650) — CrossOver Wine + Vulkan + Steam DRM
- **Rendering**: D3D11 via CrossOver Wine built-in Vulkan/D3D11 support — native Metal
- **Audio**: Working via CrossOver Wine audio bridge
- **Input**: Working via CrossOver Wine input translation
- **Launch**: CrossOver Wine + `steam://run/` for full Steam DRM authentication
- **Architecture**: 64-bit PE32+ (RE Engine)

### Elden Ring (1245620) — CrossOver Wine + MetalFX + Steam DRM
- **Rendering**: D3D12 via CrossOver Wine + Apple MetalFX spatial upscaling via `D3DM_ENABLE_METALFX=1`
- **Audio**: Working via CrossOver Wine audio bridge
- **Input**: Working via CrossOver Wine input translation + Steam Input
- **Launch**: CrossOver Wine + `steam://run/` with MetalFX and D3DMetal performance tuning
...
### CrossOver Pipeline (Among Us, Ghostrunner, RE4, Elden Ring)
```
