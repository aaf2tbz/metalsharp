...
### Among Us (945360) — external runtime Wine + Vulkan
- **Rendering**: Unity IL2CPP via external runtime Wine built-in Vulkan/D3D11 support — native Metal via MoltenVK
...
- **Launch**: external runtime Wine (`/Applications/external runtime.app/Contents/SharedSupport/external runtime/lib/wine/x86_64-unix/wine`) + per-game prefix
- **Architecture**: 64-bit PE32+ (Unity IL2CPP) — external runtime provides 64-bit Wine runtime
- **Critical**: Uses external runtime Wine (not GPTK or Wine Devel) — external runtime has better Unity IL2CPP compatibility
- **DXVK removed** — external runtime provides its own D3D/Vulkan translation, no DXVK DLLs needed in game dir
- **Required**: external runtime (`/Applications/external runtime.app/`)
- **Setup script**: `scripts/setup-amongus-deps.sh` — external runtime prefix init, DXVK DLL removal
...
### Ghostrunner (1139900) — external runtime Wine + Vulkan
- **Rendering**: D3D11 via external runtime Wine built-in Vulkan/D3D11 support — native Metal via MoltenVK
- **Audio**: Working via external runtime Wine audio bridge
- **Input**: Working via external runtime Wine input translation + Steam Input
- **Launch**: external runtime Wine + per-game prefix, DXVK DLLs removed from game dir
- **Architecture**: 64-bit PE32+ (Unreal Engine 4)
- **Critical**: Uses external runtime Wine — provides 64-bit Wine runtime with built-in Vulkan renderer
...
### DREDGE (1562430) — external runtime Wine + D3DMetal Perf + Steam DRM
- **Rendering**: D3D11 via external runtime Wine + D3DMetal performance tuning (async commit, multithreaded, skip barriers)
- **Audio**: Working via external runtime Wine audio bridge
- **Input**: Working via external runtime Wine input translation
- **Launch**: external runtime Wine + `steam://run/` with D3DMetal perf env vars
- **Architecture**: 64-bit PE32+ (Unity)
- **Engine auto-detection**: Hardcoded as SteamD3DMetalPerf
- **Required**: external runtime (`/Applications/external runtime.app/`)

### Sons of the Forest (1326470) — external runtime Wine + D3DMetal Perf + Steam DRM
- **Rendering**: D3D11/12 via external runtime Wine + D3DMetal performance tuning (async commit, multithreaded, skip barriers)
- **Audio**: Working via external runtime Wine audio bridge
- **Input**: Working via external runtime Wine input translation
- **Launch**: external runtime Wine + `steam://run/` with D3DMetal perf env vars
...
- **Status**: external runtime launches game window and loading screen, then crashes. DXVK d3d9 loads but fails at Vulkan adapter/queue enumeration through external runtime's winevulkan.
- **Blockers**: DXVK d3d9 requires proper winevulkan queue enumeration; external runtime's wrapper reports device API version 0.0.0 and no graphics queues to external DLLs
...
| DREDGE | Unity audio | via external runtime Wine | Working |
| Sons of the Forest | Unity HDRP audio | via external runtime Wine | Working |
...
| DREDGE | D3D11 → external runtime D3DMetal | 2 | external runtime Wine, D3DMetal perf, auto-detected |
| Sons of the Forest | D3D11/12 → external runtime D3DMetal | 2 | external runtime Wine, D3DMetal perf, Unity HDRP |
